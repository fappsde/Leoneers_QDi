...
MPU6050G2::MPU6050G2(int8_t addr, bool run_update_thread) {
	int status;
...
std::thread(&MPU6050G2::_update, this).detach(); //Create a seperate thread, for the update routine to run in the background, and detach it, allowing the program to continue
...
MPU6050G2::MPU6050G2(int8_t addr) : MPU6050G2(addr, true){}

void MPU6050G2::getGyroRaw(float *roll, float *pitch, float *yaw) {
...
void MPU6050G2::getGyro(float *roll, float *pitch, float *yaw) {
...
void MPU6050G2::getAccelRaw(float *x, float *y, float *z) {
...
void MPU6050G2::getAccel(float *x, float *y, float *z) {
...
void MPU6050G2::getOffsets(float *ax_off, float *ay_off, float *az_off, float *gr_off, float *gp_off, float *gy_off,int caltime) {
	...
	*ax_off = 0, *ay_off = 0, *az_off = 0; //Initialize the offsets to zero
	...
	for (int i = 0; i < caltime; i++) { //Use loop to average offsets
	...
	*gr_off = *gr_off / caltime, *gp_off = *gp_off / caltime, *gy_off = *gy_off / caltime; //Divide by number of loops (to average)
	*ax_off = *ax_off / caltime, *ay_off = *ay_off / caltime, *az_off = *az_off / caltime;

	*az_off = *az_off - ACCEL_SENS; //Remove 1g from the value calculated to compensate for gravity)
}

int MPU6050G2::getAngle(int axis, float *result) {
...
void MPU6050G2::_update() { //Main update function - runs continuously
